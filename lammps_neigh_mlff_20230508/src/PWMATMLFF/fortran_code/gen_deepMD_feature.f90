module calc_deepMD_f
   ...
   integer :: m_neigh,m_neigh_t5,emb_dim
   ...
   integer(4) :: is_type_embedding
   real(8), allocatable,dimension (:,:) :: type_embedding
   real(8), allocatable,dimension (:,:,:,:) :: s_neigh_type
...
contains
   ...
   subroutine load_model_deepMD_f(ff_idx)
      ...
      if (is_type_embedding .eq. 1) then
         ...
         do i=1,iitype_count ! used here for hybrid model, but still need to be tested
            ...
            ! type_embedding = ff(ff_idx)%dp_ff_type_embedding
         enddo
      endif
      !!< type_embedding reading end
   ...
   subroutine set_image_info_deepMD_f(ff_idx)
      ...
      m_neigh_t5 = m_neigh
      emb_dim = ff(ff_idx)%dp_ff_node_em(1)
      ...
      allocate(s_neigh_type(emb_dim,m_neigh,ntypes,natoms))
   ...
   subroutine gen_deepMD_feature(num_neigh,dR_neigh)
      ...
      if (is_type_embedding .eq. 1) then
         do iat=1,natoms
            do itype=1,ntypes
               do j=1,m_neigh
                  s_neigh_type(1,j,itype,iat)=s_neigh(j,itype,iat)
                  s_neigh_type(2:emb_dim,j,itype,iat)=type_embedding(:,itype)
               enddo
            enddo
         enddo
      endif
      ...
      !   enddo
      ! enddo
      !>> interface to python S_Rij_type
      ! do iat=1,natoms
      !   write(*,"(A, 1(I3, 1X))") "natom", iat
      !   do itype=1,ntypes
      !     do j=1,m_neigh
      !       write(*,"(6(F12.6, 2X))") s_neigh_type(:,j,itype,iat)
      !     enddo
      !   enddo
      ! enddo

      !>> deallocate arrays
      if (allocated(type_embedding)) deallocate(type_embedding)
