module calc_ftype1
    ...
    subroutine gen_feature_type1(AL,xatom)
        ...
        !write(*,*) "m_neigh,ntype,natom", m_neigh,ntype,natom 
        ...
        allocate(feat(nfeat0m,natom))         ! each note, only know its own feat
        allocate(dfeat(nfeat0m,natom,m_neigh,3))  ! dfeat is the derivative from the neighboring dR, 
        ...
        call find_neighbore(iatom,natom,xatom,AL,Rc_type,num_neigh,list_neigh, &
                            dR_neigh,iat_neigh,ntype,iat_type, &
                            m_neigh,Rc_M,map2neigh_M,list_neigh_M, &
                            num_neigh_M,iat_neigh_M,inode,nnodes)

        !call cpu_time(timefinish)

        !write(*,*) "nblist calc time:",timefinish - timestart
        ...
        do iat=1,natom
            ! go through the prescribed atoms
