module dp_ff_mod
   ...
   type single_ff
      ...
      real(8), allocatable,dimension(:,:,:) :: dp_ff_Wij_em_type
      real(8), allocatable,dimension(:,:) :: dp_ff_B_em_type
      ...
      ! type_embedding file
      integer(4) dp_ff_is_type_embedding
      integer(4) dp_ff_iat_type_embedding(10)
      real(8), allocatable, dimension(:,:) :: dp_ff_type_embedding
   end type single_ff
...
contains
   ...
   subroutine dp_ff_load(name_ptr, ff_idx, slen, ocut, m_neigh) bind(c,name="dp_ff_load")
      ...
      if(ff(ff_idx)%dp_ff_ntype_pair.ne.ff(ff_idx)%dp_ff_num_type**2 .and. &
         ff(ff_idx)%dp_ff_node_em(1).eq.1) then
      ...
      allocate(ff(ff_idx)%dp_ff_Wij_em_type( ff(ff_idx)%dp_ff_nodeMM_em, &
         ff(ff_idx)%dp_ff_nodeMM_em, &
         ff(ff_idx)%dp_ff_nlayer_em))

      allocate(ff(ff_idx)%dp_ff_B_em_type( ff(ff_idx)%dp_ff_nodeMM_em, &
         ff(ff_idx)%dp_ff_nlayer_em))

      if (ff(ff_idx)%dp_ff_node_em(1).eq.1) then
         ...
         do ll=1,ff(ff_idx)%dp_ff_nlayer_em
            do j1=1,ff(ff_idx)%dp_ff_node_em(ll)
               read(10,*) (ff(ff_idx)%dp_ff_Wij_em_type(j1,j2,ll), &
                  j2=1,ff(ff_idx)%dp_ff_node_em(ll+1))
            enddo
            read(10,*) (ff(ff_idx)%dp_ff_B_em_type(j2,ll), &
               j2=1,ff(ff_idx)%dp_ff_node_em(ll+1))
         enddo
      endif
      ...
      !!> reading type_embedding.in part
      read(10,*) ff(ff_idx)%dp_ff_is_type_embedding
      allocate(ff(ff_idx)%dp_ff_type_embedding(ff(ff_idx)%dp_ff_node_em(1)-1, ff(ff_idx)%dp_ff_num_type))
      if (ff(ff_idx)%dp_ff_is_type_embedding == 1) then
         do i = 1, ff(ff_idx)%dp_ff_num_type
            read(10,*) ff(ff_idx)%dp_ff_iat_type_embedding(i)
            read(10,*) (ff(ff_idx)%dp_ff_type_embedding(j,i), j = 1, ff(ff_idx)%dp_ff_node_em(1)-1)
         enddo
      endif
      !!< reading type_embedding.in part end
   ...
   subroutine dp_ff_deallocate(ff_idx) bind(c,name="dp_ff_deallocate")
      ...
      deallocate(ff(ff_idx)%dp_ff_Wij_em_type)
      deallocate(ff(ff_idx)%dp_ff_B_em_type)
      ...
      deallocate(ff(ff_idx)%dp_ff_type_embedding)
