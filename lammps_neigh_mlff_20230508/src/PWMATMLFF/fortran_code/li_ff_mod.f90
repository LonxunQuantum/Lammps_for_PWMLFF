...
contains
   ...
   subroutine li_ff_load(name_ptr, ff_idx, slen, ocut) bind(c,name="li_ff_load")
      ...
      do kk = 1, ff(ff_idx)%ff_nfeat_type
         ...
         ! for feature5
         if (ff(ff_idx)%ff_ifeat_type(kk).eq.5) then
            call f5(ff_idx,ocut)
         endif
         ! for feature6
   ...
   subroutine f5(ff_idx,ocut)
      ...
      do itype=1,ff(ff_idx)%ff_num_type
         ...
         read(10,*) ff(ff_idx)%ff_Rc_type(itype),ff(ff_idx)%ff_Rm_type(itype)
         if(ff(ff_idx)%ff_Rc_type(itype).gt.ff(ff_idx)%ff_Rc_M) then
            write(6,*) "Rc_type must be smaller than Rc_M, gen_3b_feature.in", &
               itype,ff(ff_idx)%ff_Rc_type(itype),ff(ff_idx)%ff_Rc_M
            stop
         endif
         read(10,*) ff(ff_idx)%ff_nMTP_type(itype) ! number of MTP type (line, each line can be expanded to many MTP)

         numCC=0
         do kkk=1,ff(ff_idx)%ff_nMTP_type(itype)
            read(10,*) ff(ff_idx)%ff_MTP_num
            backspace(10)
            if(ff(ff_idx)%ff_MTP_num.gt.4) then
               write(6,*) "we only support contraction up to 4 tensors,stop",ff(ff_idx)%ff_MTP_num
               stop
            endif
            ! Cannot do double loop with txt
            if(ff(ff_idx)%ff_MTP_num.eq.1) then
               read(10,*,iostat=ierr) ff(ff_idx)%ff_MTP_num,(ff(ff_idx)%ff_mu(i),i=1,ff(ff_idx)%ff_MTP_num),&
                  (ff(ff_idx)%ff_rank(i),i=1,ff(ff_idx)%ff_MTP_num),txt,&
                  (ff(ff_idx)%ff_ind(j,1),j=1,ff(ff_idx)%ff_rank(1)),txt
            elseif(ff(ff_idx)%ff_MTP_num.eq.2) then
               read(10,*,iostat=ierr) ff(ff_idx)%ff_MTP_num,(ff(ff_idx)%ff_mu(i),i=1,ff(ff_idx)%ff_MTP_num),&
                  (ff(ff_idx)%ff_rank(i),i=1,ff(ff_idx)%ff_MTP_num),txt,&
                  (ff(ff_idx)%ff_ind(j,1),j=1,ff(ff_idx)%ff_rank(1)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,2),j=1,ff(ff_idx)%ff_rank(2)),txt
            elseif(ff(ff_idx)%ff_MTP_num.eq.3) then
               read(10,*,iostat=ierr) ff(ff_idx)%ff_MTP_num,(ff(ff_idx)%ff_mu(i),i=1,ff(ff_idx)%ff_MTP_num),&
                  (ff(ff_idx)%ff_rank(i),i=1,ff(ff_idx)%ff_MTP_num),txt,&
                  (ff(ff_idx)%ff_ind(j,1),j=1,ff(ff_idx)%ff_rank(1)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,2),j=1,ff(ff_idx)%ff_rank(2)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,3),j=1,ff(ff_idx)%ff_rank(3)),txt
            elseif(ff(ff_idx)%ff_MTP_num.eq.4) then
               read(10,*,iostat=ierr) ff(ff_idx)%ff_MTP_num,(ff(ff_idx)%ff_mu(i),i=1,ff(ff_idx)%ff_MTP_num),&
                  (ff(ff_idx)%ff_rank(i),i=1,ff(ff_idx)%ff_MTP_num),txt,&
                  (ff(ff_idx)%ff_ind(j,1),j=1,ff(ff_idx)%ff_rank(1)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,2),j=1,ff(ff_idx)%ff_rank(2)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,3),j=1,ff(ff_idx)%ff_rank(3)),txt,&
                  txt,(ff(ff_idx)%ff_ind(j,4),j=1,ff(ff_idx)%ff_rank(4)),txt
            endif
            ...
            do i=1,ff(ff_idx)%ff_MTP_num
               do j=1,ff(ff_idx)%ff_rank(i)
                  indi(j,i)=ff(ff_idx)%ff_ind(j,i)/10
                  indj(j,i)=ff(ff_idx)%ff_ind(j,i)-indi(j,i)*10
               enddo
            enddo
            ...
            call get_expand_MT(ff(ff_idx)%ff_num_type,ff(ff_idx)%ff_MTP_num,indi,indj,ff(ff_idx)%ff_mu,ff(ff_idx)%ff_rank,numC,jmu_b,itype_b)
            ! numC is the MTP of this line (after expansion, due mu, and ntypes)

            numc0=1
            do i=1,ff(ff_idx)%ff_MTP_num
               numc0=(1+ff(ff_idx)%ff_mu(i))*numc0*ff(ff_idx)%ff_num_type
            enddo
            ...
            do kk=1,numC
               kkc=numCC+kk
               numT_all(kkc,itype)=ff(ff_idx)%ff_MTP_num
               do i=1,ff(ff_idx)%ff_MTP_num
                  rank_all(i,kkc,itype)=ff(ff_idx)%ff_rank(i)
                  mu_all(i,kkc,itype)=ff(ff_idx)%ff_mu(i)
