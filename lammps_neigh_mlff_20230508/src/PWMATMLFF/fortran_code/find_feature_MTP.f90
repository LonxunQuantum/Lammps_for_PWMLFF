subroutine find_feature_MTP(num_neigh,dR_neigh,m_neigh,list_neigh, &
   numCC_type,numT_all,mu_all,rank_all,jmu_b_all,itype_b_all,indi_all,indj_all, &
   feat_all,dfeat_all,nfeat0m,Rc,Rm)
   ...
   integer, intent(in) :: numCC_type(10),numT_all(20000,10)
   integer, intent(in) :: rank_all(4,20000,10),mu_all(4,20000,10),jmu_b_all(4,20000,10),itype_b_all(4,20000,10)
   integer, intent(in) :: indi_all(5,4,20000,10),indj_all(5,4,20000,10)
   integer, intent(in) :: nfeat0m
   real(8), intent(out) :: feat_all(nfeat0m,natoms)
   real(8), intent(out) :: dfeat_all(nfeat0m,natoms,m_neigh,3)
   real(8), intent(in) :: Rc(ntypes),Rm(ntypes)
   ...
   integer :: i,ii,j,jj,kk,num,iat,iat1,itype
   real(8) :: d,dd
   real(8) :: pi,pi2
   integer :: itype0
   ...
   real(8) dx(3)
   integer imu,i1,i2,i3,i4,num_tot,indc,iii,rankT
   ...
   do 3000 iat=1,natoms
      ...
      allocate(tensor(81,0:mumx,ntypes,0:4))
      allocate(dtensor(3,m_neigh,81,0:mumx,ntypes,0:4))
      tensor=0.d0
      dtensor=0.d0
